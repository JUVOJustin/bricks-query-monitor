...
namespace Bricks_Query_Monitor\Cli;
...
class Setup {
	...
	public function __invoke( array $args, array $assoc_args ): void {
		...
		// Replace in files
		if (
			! $this->replaceInFiles( 'bricks-query-monitor', $this->slug, [ '.*\.php', '.*\.js', '.*\.json' ] )
			|| ! $this->replaceInFiles( 'bricks_query_monitor', str_replace( '-', '_', $this->slug ), [ '.*\.php' ] )
			|| ! $this->replaceInFiles( 'Bricks_Query_Monitor', $this->namespace, [ '.*\.php', '.*\.json' ] )
			|| ! $this->replaceInFiles( 'BRICKS_QUERY_MONITOR', strtoupper( $this->namespace ), [ '.*\.php', '.*\.json' ] )
			|| ! $this->replaceInFiles( 'Bricks Query Monitor', $this->name, [ '.*\.php', '.*README\.txt' ] )
		) {
	...
	private function rename_files(): void {
		if (
			! rename( $this->path . '/src/Bricks_Query_Monitor.php', $this->path . "/src/$this->namespace.php" )
			|| ! rename( $this->path . '/bricks-query-monitor.php', $this->path . "/$this->slug.php" )
		) {
